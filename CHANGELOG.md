...
- **路径防护：拦截日志文件名只取文件名部分**（`[intercept_log].file_name`）。
  以前该值会直接拼在插件数据目录后面，配置写成 `../evil.jsonl`、`sub/x.jsonl` 或绝对路径
  （`D:\evil.jsonl`）时，日志会落到 `data_dir` **之外**；`..` 更是直接指向父目录。
  现在统一剥离目录，并把 `''` / `.` / `..` 这类"仍然是目录"的名字回退为默认 `intercept.jsonl`
  ——日志本体与轮转备份（`.1`/`.2`/`.3`）**永远只写在插件 `data_dir` 内**。
- 配置值被规范化时，加载日志会给出一条 warning（**不静默改写**），便于发现写错的配置。
- `intercept_log.py` 的 `InterceptLogger` **内部同样设这道防线**（防御性双重保险：
  任何调用方直接构造写入器也绕不出去）。
- 版本号与 `config_version` 同步升至 `1.1.1`（配置字段无增删，行为仅此一处收紧）。
